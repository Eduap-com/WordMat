...
//    }; // Floating Point error
  ...
  //      execl ("/bin/sh", MaximaPath ,(char *) NULL);
        execl (MaximaPath, MaximaPath ,(char *) NULL); // exec'd applications inherit stdiinput and stdoutput from calling process // if succesful the childprocess is left and the following lines never run
...
long StartShell()
{
    ...
    if (ChildPID < 0){
        ...
        execl ("/bin/sh", "MyShell" ,"-i",(char *) NULL);

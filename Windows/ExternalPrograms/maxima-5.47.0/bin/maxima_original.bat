...
if not exist "%maxima_prefix%" set maxima_prefix=D:\PROGRA~1\WordMat\MAXIMA~1.1
if not exist "%maxima_prefix%" set maxima_prefix=C:\PROGRA~1\WordMat\Maxima-5.47.0
if not exist "%maxima_prefix%" set maxima_prefix=C:\PROGRA~2\WordMat\Maxima-5.47.0
if not exist "%maxima_prefix%" set maxima_prefix=%ProgramFiles(x86)%\WordMat\Maxima-5.47.0
if not exist "%maxima_prefix%" set maxima_prefix=%ProgramFiles%\WordMat\Maxima-5.47.0
...
/tmp/maxima-5.47.0/crosscompile-windows/build/wine-clisp.sh %lisp_options% -q -M "%maxima_image_base%.mem" "" -- %*
goto :EOF

:clisp_autotools
rem ./configure sets a variable /tmp/maxima-5.47.0/crosscompile-windows/build/wine-clisp.sh which might be used instead of lisp.exe,
rem but when Maxima is crosscompiled on Linux, a shell script is used as 'crosscompiler',
rem which would be inserted here ==> do not use the variable /tmp/maxima-5.47.0/crosscompile-windows/build/wine-clisp.sh but just lisp.exe.
...
rem ./configure sets a variable /tmp/maxima-5.47.0/crosscompile-windows/build/wine-sbcl.sh which might be used instead of sbcl.exe,
rem but when Maxima is crosscompiled on Linux, a shell script is used as 'crosscompiler',
rem which would be inserted here ==> do not use the variable /tmp/maxima-5.47.0/crosscompile-windows/build/wine-sbcl.sh but just sbcl.exe.
sbcl.exe --core "%maxima_image_base%.core" --noinform %lisp_options% --end-runtime-options --eval %start_maxima% --end-toplevel-options %*
goto :EOF
...
openmcl -I "%maxima_image_base%" %lisp_options% -e %start_maxima% -- %*
goto :EOF
...
dx86cl64 -I "%maxima_image_base%" %lisp_options% -e %start_maxima% -- %*
goto :EOF

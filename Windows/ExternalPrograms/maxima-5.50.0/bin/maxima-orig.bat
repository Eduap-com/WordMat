...
set "lisp=sbcl"
set lisp_options=
set "version=branch_5_50_base_9_gf03405fbf_dirty"
...
clisp.exe %lisp_options% -q -M "%maxima_image_base%.mem" "" -- %*
goto :EOF

:clisp_autotools
set "path=%maxima_imagesdir%/binary-%lisp%;%maxima_prefix%/clisp-2.49;%path%"
...
:doecl
chcp 65001 >nul
set "path=%maxima_prefix%/ecl;%maxima_prefix%/w64devkit/bin;%path%"
set "MAXIMA_IMAGESDIR_BIN=%maxima_imagesdir%/binary-%lisp%"
ecl  --eval "(load \"%MAXIMA_IMAGESDIR_BIN%/defsystem.lisp\")" --eval "(mk:add-registry-location \"%MAXIMA_IMAGESDIR_BIN%\")" --eval "(funcall (intern (symbol-name :operate-on-system) :mk) \"maxima\" :load :verbose nil)" --eval "(cl-user::run)" -- %*
goto :EOF
...
rem set codepage to unicode
chcp 65001 >nul
wx86cl64.exe -I "%maxima_image_base%.image" %lisp_options% -e %start_maxima% -- %*
goto :EOF
